...
export default function MatchPage() {
  ...
  const uniqueInterests = Array.from(
    new Set(data.flatMap((local) => local.interests))
  ); // Unique interests from locals
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          //waits until all the data is loaded to process it
  ...
  // Filters
  useEffect(() => {
    //useEffect - 'activates' the code whenver something changes , user use filter
    const applyFilter = () => {
      let filtered = data;

      if (selectedGenders.length > 0) {
        // if the user wants to filter then the code does its jobb
        filtered = filtered.filter((local) =>
          selectedGenders.includes(local.gender)
        );
      }

      filtered = filtered.filter(
        (local) =>
          local.age >= filterAgeRange[0] && local.age <= filterAgeRange[1]
      ); // checks if the local is between the minimum 0 and the maximum 1 age that the use chose

      if (selectedInterests.length > 0) {
        filtered = filtered.filter((local) =>
          local.interests.some((interest) =>
  ...
  const handleGenderChange = (gender) => {
    setSelectedGenders(
      (
        prev //prev is currently chosen genders
      ) =>
        prev.includes(gender)
          ? prev.filter((g) => g !== gender)
          : [...prev, gender] //if the gender is already on the list then the code creates a new list without it (ofc if the users unclick the gender), if the gender is not on the list of chosen fgenders and the user clicks on it the the code adds the gender to the list. It is so simply Idk why I am writing that much
    );
  };
  ...
  const allLocalsViewed =
    !noLocalsMatchFilter && currentIndex >= filteredData.length;
  ...
  return (
    <section className="page1">
      {!allLocalsViewed && ( //if it false then the code runs
        <div className="filter-section">
          ...
          <div className="filter-container">
            <button className="filter-button" onClick={toggleGenderDropdown}>
              Gender
            </button>
          ...
          {/* Age Filter Dropdown */}
          <div className="filter-container">
            <button className="filter-button" onClick={toggleAgeDropdown}>
              Age
            </button>
              ...
              <div className="age-range-dropdown">
                ...
                <div className="slider-container">
                  <Range
                    ...
                    renderTrack={(
                      { props, children } //styling the track
                    ) => (
                      <div
                        {...props}
                        style={{
                    ...
                    renderThumb={(
                      { props } //styling those two things on the sides
                    ) => (
                      <div
                        {...props}
                        style={{
          ...
          <div className="filter-container">
            <button className="filter-button" onClick={toggleInterestsDropdown}>
              Activities
            </button>
      ...
      <div className="container1">
        {loading && <p>Loading...</p>}{" "}
        {/* it will show loading if its loading */}
        {error && <p>Error: {error}</p>}{" "}
        {/* it will show error if theres an erorr */}
        ...
        {!allLocalsViewed &&
          currentIndex < filteredData.length && ( //it will show options to scroll if all the locals weren viewed and if there is still someone considering filtered data
